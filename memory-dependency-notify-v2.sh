...
subscribe() {
    local event_name="$1"
    local subscriber="${2:-${MY_AGENT:-unknown}}"
    ...
    if [[ -z "$event_name" ]]; then
        ...
        echo -e "${YELLOW}Example: subscribe api-deployment agent-frontend completed${NC}"
        return 1
    fi
...
publish() {
    ...
    local publisher="${4:-${MY_AGENT:-unknown}}"
...
my_subscriptions() {
    local subscriber="${MY_AGENT:-unknown}"
...
wait_for() {
    ...
    local subscriber="${MY_AGENT:-wait-$$}"
...
${BLUE}subscribe${NC} <event-name> [subscriber-id] [notify-when]
    ...
    Example: subscribe api-deployment agent-frontend completed

${BLUE}unsubscribe${NC} <event-name> [subscriber-id]
    Unsubscribe from an event
    Example: unsubscribe api-deployment agent-frontend
...
${BLUE}list-subscriptions${NC} [subscriber-id]
    ...
    Example: list-subscriptions agent-frontend
...
${BLUE}my-subscriptions${NC}
    Show subscriptions for current Agent (\$MY_AGENT)
...
${GREEN}WORKFLOW EXAMPLE:${NC}
    ...
    # Agent A (Frontend) needs API to be ready
    MY_AGENT=agent-frontend $0 subscribe api-deployment completed

    # Agent B (Backend) deploys API
    MY_AGENT=agent-backend $0 publish api-deployment started "Deploying..."
    # ... do deployment work ...
    MY_AGENT=agent-backend $0 publish api-deployment completed "API live at api.blackroad.io"

    # Agent A automatically gets notified! 🔔
...
${GREEN}USE CASES:${NC}
    ...
    • Async handoffs between BlackRoad Agents
