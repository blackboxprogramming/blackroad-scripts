...
# SUBDOMAIN MASS GENERATOR (Fixed for zsh compatibility)
...
# Function to capitalize first letter (zsh/bash compatible)
capitalize() {
...
generate_subdomain_site() {
    ...
    local subdomain_title=$(capitalize "$subdomain")

    local dir="${HOME}/blackroad-subdomains/${parent_domain}/${subdomain}"
    mkdir -p "$dir"
...
<head>
    ...
    <title>SUBDOMAIN_TITLE - PARENT_DOMAIN</title>
    <style>
...
HTMLEOF
    ...
    sed -i '' "s/SUBDOMAIN_TITLE/$subdomain_title/g" "$dir/index.html"
    sed -i '' "s/FULL_DOMAIN/$full_domain/g" "$dir/index.html"
    sed -i '' "s/PARENT_DOMAIN/$parent_domain/g" "$dir/index.html"
...
echo ""
echo "Total subdomains created: $(find ${HOME}/blackroad-subdomains -name "index.html" 2>/dev/null | wc -l)"
